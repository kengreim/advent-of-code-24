...
fn parse_program(input: &str) -> (Vec<u32>, (u32, u32, u32)) {
    let register_a: u32 = REGISTER_A_RE
        .captures(&input)
        .unwrap()
        .get(1)
        .unwrap()
        ...
        .parse()
        .unwrap();
    let register_b: u32 = REGISTER_B_RE
        .captures(&input)
        .unwrap()
        .get(1)
        .unwrap()
        ...
        .parse()
        .unwrap();
    let register_c: u32 = REGISTER_C_RE
        .captures(&input)
        .unwrap()
        .get(1)
        .unwrap()
...
fn execute_program(program: &[u32], registers: (u32, u32, u32)) -> Vec<u32> {
    ...
    while i_pointer < program.len() {
        ...
        match opcode {
            0 => {
                ...
                let denominator = 2_u32.pow(get_combo_operand_val(
                    operand,
...
fn get_combo_operand_val(operand: u32, registers: (u32, u32, u32)) -> u32 {
    match operand {
        0 | 1 | 2 | 3 => operand,
        4 => registers.0,
        5 => registers.1,
